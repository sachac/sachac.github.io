...
#+CALL: graph-from-tables[:file "goals.svg" :cmdline "-Kdot -Tsvg -Gdpi=100 -Gsize=8,8 -Grankdir=LR"](fillcolumn=20,id="G") :results silent :exports none
         ...
         (mapconcat
          (lambda (x)
            (sacha/elgraphviz-node
             ...
             :tooltip (elt x 4)))
          (cdr (assoc 'nodes map)) "\n")))
     (org-babel-execute:dot (sacha/org-map-to-graphviz (sacha/org-map-goals tag) fill-column id) params)
#+END_SRC
